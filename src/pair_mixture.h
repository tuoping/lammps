...
PairStyle(mixture,PairMixture)

#else

#ifndef LMP_PAIR_MIXTURE_H
#define LMP_PAIR_MIXTURE_H
...
class PairMixture : public Pair {
 ...
 protected:
  ...
  class ComputeCoordAtom *c_coordatom;
  char *id_coordatom;
  double coord_thres;
  double thick_trans;
  int ssa;
  int communicateStage;
  int timestep_pm;
  int Updatenmax;
  FILE *of_wfunc;
  FILE *of_imp;
  int nimpurity, nimpurity_total;
  template <typename T> void softmin(const typename::std::vector<T> &v, typename::std::vector<T> &s);
  template <typename T> void softmin_deriv(const typename::std::vector<T> &s, typename::std::vector<std::vector<T> > &d);
  template <typename T> void sortIndex(typename::std::vector<T> &b, int m[], int n);
  int pgsize,oneatom;           // copy of settings in Neighbor
  int dnum;                     // num of values per neighbor
  int ndpsamples;
  // MyPage<double> *dpage_w;   // pages of partner values
 private:
  int me,nprocs;
};
