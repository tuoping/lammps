    ...
    if (impurity[i] == 1.0){
        ...
        for (jj = 0; jj < jnum; jj++) {
            ...
            if (impurity[j] == 1.0){
                ...
                if (rsq < mindsq) mindsq = rsq;
                if (rsq > maxdsq) maxdsq = rsq;
            }
        }
        if (setwflag != 0) {
            double mind = sqrt(mindsq);
            if (mind < thick_trans){
                force->wfunc[i] = 0.5 * (1.0 + cos(M_PI * sqrt(mindsq) / thick_trans));
                force->ndeepmd_region ++;
            }
        }
    }
...
void PairMixture::unpack_forward_comm(int n, int first, double *buf)
{
  ...
  if (communicateStage == 1){
    m = 0;
    last = first + n;
    for (i = first; i < last; i++) impurity[i] = buf[m++];
  }else{
    if (communicateStage == 2){
      m = 0;
      last = first + n;
      for (i = first; i < last; i++) force->wfunc[i] = buf[m++];
    }
  }
}
  ...
  // if (force->newton_pair) comm->reverse_comm_pair(this);
    ...
    for (int ii = 0; ii < atom->nmax; ++ii){
      for (int dd = 0; dd < 3; ++dd){
          atom->f[ii][dd] = force->f_deepmd[ii*3+dd] * force->wfunc[ii] + atom->f[ii][dd] * (1.0 - force->wfunc[ii]);
      }
    }
