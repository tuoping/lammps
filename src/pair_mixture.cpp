...
PairMixture::PairMixture(LAMMPS *lmp) : Pair(lmp),
  styles(NULL), keywords(NULL), multiple(NULL), nmap(NULL),
  map(NULL), special_lj(NULL), special_coul(NULL), compute_tally(NULL), impurity(NULL)
{
  nstyles = 0;
  coord_thres = 0.0;
  thick_trans = 0.0;
  ...
  dnum = 3;
  of_wfunc = fopen("wfunc.dat","w");
  of_imp = fopen("imp.dat","w");
...
PairMixture::~PairMixture()
{
  ...
  memory->destroy(force->wfunc);
  // memory->destroy(force->neiofbuf);
  memory->destroy(impurity);
  memory->destroy(force->f_deepmd);
}
...
------------------------------------------------------------------------- */
void PairMixture::get_impurity()
{
  int i, ii,inum;
  int *ilist;
  double *cvec;
  // Compute->invoked_peratom = update->ntimestep;
  memset(impurity, 0, sizeof(double) * nmax);
  
  inum = list->inum;
  ilist = list->ilist;
  
  int icoordatom = modify->find_compute(id_coordatom);
  c_coordatom = (ComputeCoordAtom*)(modify->compute[icoordatom]);
  if (!(c_coordatom->invoked_flag & c_coordatom->invoked_peratom)) {
    c_coordatom->compute_peratom();
    c_coordatom->invoked_flag |= c_coordatom->invoked_peratom;
  }
  cvec = c_coordatom->vector_atom;
  double **x = atom->x;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (cvec[i] != coord_thres){
    if (cvec[i] < coord_thres){
      impurity[i] = 1;
      nimpurity ++;
    }
    fprintf(of_imp, "%d  %15.5f  %20.15f  %20.15f\n", i, atom->x[i][0], cvec[i], impurity[i]);
  }
  
  std::cout<<"nimpurity = "<<nimpurity<<std::endl;
  MPI_Allreduce(&nimpurity,&nimpurity_total,1,MPI_LMP_BIGINT,MPI_SUM,world);
  std::cout<<"nimpurity_total = "<<nimpurity_total<<std::endl;
}
...
template <typename T>
void PairMixture::softmin(const typename::std::vector<T> &v, typename::std::vector<T> &s){
    double sum=0.0;
...
template <typename T>
void PairMixture::softmin_deriv(const typename::std::vector<T> &s, typename::std::vector<std::vector<T> > &d){
    int n = s.size();
    for (int i = 0; i < n; i++){
        d[i].resize(n);
        for (int j = 0; j < n; j++){
...
template <typename T>
void PairMixture::sortIndex(typename::std::vector<T> &b, int m[], int n)
{
    int temp_int;
    T temp_T;
    for (int i = 0; i < n; i++)
...
void PairMixture::build_wfunc()
{
  // build wfunc
  int i, j, ii,jj,inum,jnum;
  double xtmp,ytmp,ztmp,delx,dely,delz,rsq, r;
  double mdelx,mdely,mdelz;
  ...
  if (atom->nmax > nmax) {
    if (nmax > 0){
        ...
        memory->destroy(impurity);
        memory->destroy(force->wfunc);
        memory->destroy(force->grad_wfunc);
        memory->destroy(force->neiofbuf);
        memory->destroy(force->f_deepmd);
        memory->destroy(force->f_rep);
        memory->destroy(force->dpneigh);
        memory->destroy(force->grad_dpneigh);
        memory->destroy(force->numdpneigh);
    }
    nmax = atom->nmax;
    Updatenmax = 1;
    memory->create(force->wfunc, nmax, "force::wfunc");
    memory->create(force->grad_wfunc, nmax, 3, "force::grad_wfunc");
    memory->create(force->neiofbuf, nmax, "force::neiofbuf");

    memory->create(impurity,nmax,"pair_mixture:impurity");

    memory->create(force->f_deepmd,nmax*3,"force:f_deepmd");
    memory->create(force->f_rep, nmax, 3, "force::f_rep");
    force->dpneigh = (int **) memory->smalloc(nmax*sizeof(int *),
                                   "force:dpneigh");
    force->grad_dpneigh = (double **) memory->smalloc(nmax*sizeof(double *),
                                   "force:grad_dpneigh");

    memory->create(force->numdpneigh,nmax,"force:numdpneigh");
    get_impurity();
    communicateStage = 1;
    comm->forward_comm_pair(this);
  }

  memset(force->wfunc, 0, sizeof(double) * nmax);
  memset(&force->grad_wfunc[0][0], 0, sizeof(double) * nmax*3);
  memset(force->numdpneigh, 0, sizeof(int) * nmax);
  memset(force->neiofbuf, 0, sizeof(int) * nmax);
  memset(force->f_deepmd, 0, sizeof(double) * nmax*3);
  memset(&force->f_rep[0][0], 0, sizeof(double) * nmax*3);
  ...
  int nlocal = atom->nlocal;
  //
  int **dpneigh = force->dpneigh;
  double **grad_dpneigh = force->grad_dpneigh;

  force->ndeepmd_region = 0;
  force->nhalfdeepmd_region = 0;

  // fprintf(of_imp, ">>>\n");
  // for (ii = 0; ii < inum; ii++) {
  //   i = ilist[ii];
  //   fprintf(of_imp, "%d  %15.5f  %20.15f  %20.15f\n", i, atom->x[i][0], impurity[i]);
  // }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (impurity[i] == 1.0){
        ...
        std::vector<double> dpni_r(jnum);
        std::vector<std::vector<double > > dpni_x(jnum);
        int n = 0;
        char nptrname[30];
        sprintf(nptrname, "force:dpneigh_%d", i);
        if (Updatenmax)
            memory->create(dpneigh[i], jnum, nptrname);
        else
            memory->grow(dpneigh[i], jnum, nptrname);
        for (jj = 0; jj < jnum; jj++) {
            j = jlist[jj];
            if (impurity[j] == 1.0){
                delx = xtmp - x[j][0];
                ...
                r = sqrt(rsq);
                setwflag = 1;
                dpni_r[n] = r/thick_trans;
                dpni_x[n].resize(3);
                dpni_x[n][0] = delx/r/thick_trans;
                dpni_x[n][1] = dely/r/thick_trans;
                dpni_x[n][2] = delz/r/thick_trans;
                dpneigh[i][n++] = j;
            }
        }
        ...
        if (setwflag != 0) {
            force->ndeepmd_region ++;
            force->numdpneigh[i] = n;
            if(n>nsize) nsize = n;
            // std::cout<<"numdpneigh = "<<n<<std::endl;
            memory->grow(dpneigh[i], n, nptrname);
            dpni_r.resize(n);
            dpni_x.resize(n);
            std::vector<double > softmin_r(dpni_r);
            softmin(dpni_r, softmin_r);
            
            // std::cout<<"softmin_r::"<<std::endl;
            // for(jj = 0; jj < n; jj++){
            //     j = dpneigh[i][jj];
            //     std::cout<<i<<"  "<<j<<"    "<<dpni_r[jj]<<"    "<<softmin_r[jj]<<std::endl;
            // }

            std::vector<std::vector<double > > softmin_deriv_r(n);
            softmin_deriv(softmin_r, softmin_deriv_r);
            // std::cout<<"softmin_deriv_r::"<<std::endl;
            // for(jj = 0; jj < n; jj++){
            //   for(int kk = 0; kk < n; kk++)
            //     std::cout<<softmin_deriv_r[jj][kk]<<"  ";
            //   std::cout<<std::endl;
            // }

            double mind = 0.0;
            for (int k = 0; k < n; k++) {
                mind += softmin_r[k] * dpni_r[k];
            }
            // std::cout<<"mind = "<<mind<<std::endl;
            force->wfunc[i] = 0.5 * (1.0 + cos(M_PI * mind));
            // std::cout<<"i = "<<i<<std::endl;
            sprintf(nptrname, "force:grad_dpneigh_%d", i);
            if (Updatenmax)
                memory->create(grad_dpneigh[i], n*3, nptrname);
            else
                memory->grow(grad_dpneigh[i], n*3, nptrname);
            memset(grad_dpneigh[i], 0, n*3*sizeof(double));
            for (int k = 0; k < n; k++) {
              for (int l = 0; l < n; l++) {
                force->grad_wfunc[i][0] += -0.5 * M_PI * sin(M_PI * mind) * softmin_deriv_r[l][k] * dpni_x[k][0] * dpni_r[l];
                force->grad_wfunc[i][1] += -0.5 * M_PI * sin(M_PI * mind) * softmin_deriv_r[l][k] * dpni_x[k][1] * dpni_r[l];
                force->grad_wfunc[i][2] += -0.5 * M_PI * sin(M_PI * mind) * softmin_deriv_r[l][k] * dpni_x[k][2] * dpni_r[l];

              }
              force->grad_wfunc[i][0] += -0.5 * M_PI * sin(M_PI * mind) * softmin_r[k] * dpni_x[k][0];
              force->grad_wfunc[i][1] += -0.5 * M_PI * sin(M_PI * mind) * softmin_r[k] * dpni_x[k][1];
              force->grad_wfunc[i][2] += -0.5 * M_PI * sin(M_PI * mind) * softmin_r[k] * dpni_x[k][2];
            }
            // std::cout<<force->grad_wfunc[i][0]<<"  "<<force->grad_wfunc[i][1]<<"  "<<force->grad_wfunc[i][2]<<std::endl;
            for (int k = 0; k < n; k++){
              for (int l = 0; l < n; l++) {
                force->grad_dpneigh[i][k*3+0] -= -0.5 * M_PI * sin(M_PI * mind) * softmin_deriv_r[l][k] * dpni_x[k][0] * dpni_r[l];
                force->grad_dpneigh[i][k*3+1] -= -0.5 * M_PI * sin(M_PI * mind) * softmin_deriv_r[l][k] * dpni_x[k][1] * dpni_r[l];
                force->grad_dpneigh[i][k*3+2] -= -0.5 * M_PI * sin(M_PI * mind) * softmin_deriv_r[l][k] * dpni_x[k][2] * dpni_r[l];
              }
              force->grad_dpneigh[i][k*3+0] -= -0.5 * M_PI * sin(M_PI * mind) * softmin_r[k] * dpni_x[k][0];
              force->grad_dpneigh[i][k*3+1] -= -0.5 * M_PI * sin(M_PI * mind) * softmin_r[k] * dpni_x[k][1];
              force->grad_dpneigh[i][k*3+2] -= -0.5 * M_PI * sin(M_PI * mind) * softmin_r[k] * dpni_x[k][2];
              // std::cout<<"k = "<<k<<"    "<<force->grad_dpneigh[i][k*3+0]<<std::endl;
            }
  ...
  // for (ii = 0; ii < inum; ii++) {
  //   i = ilist[ii];
  //   fprintf(of_wfunc, "%4d    %20.10f    %20.10f  %20.10f  %20.10f\n", i, force->wfunc[i], force->grad_wfunc[i][0], force->grad_wfunc[i][1], force->grad_wfunc[i][2]);
  // }

  communicateStage = 2;
  comm_forward = 5 + nsize * 4;
  std::cout<<"comm_forward = "<<comm_forward<<std::endl;
  comm->forward_comm_pair(this);
  std::cout<<"done forward_comm_pair"<<std::endl;
}



int PairMixture::pack_forward_comm(int n, int *list, double *buf,
                               int /*pbc_flag*/, int * /*pbc*/)
{
  int i,j,m;

  if (communicateStage == 1){
    m = 0;
    for (i = 0; i < n; i++) {
      j = list[i];
      buf[m++] = impurity[j];
    ...
    if (communicateStage == 2){
        m = 0;
        for (i = 0; i < n; i++) {
          j = list[i];
          buf[m++] = force->wfunc[j];
          buf[m++] = force->grad_wfunc[j][0];
          buf[m++] = force->grad_wfunc[j][1];
          buf[m++] = force->grad_wfunc[j][2];
          buf[m++] = (double)force->numdpneigh[j];
          for (int kk = 0; kk < force->numdpneigh[j]; kk++){
             buf[m++] = (double)force->dpneigh[j][kk];
             buf[m++] = force->grad_dpneigh[j][kk*3+0];
             buf[m++] = force->grad_dpneigh[j][kk*3+1];
             buf[m++] = force->grad_dpneigh[j][kk*3+2];
          }
        }
        return m;
    }
...
void PairMixture::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  ...
  if (communicateStage == 1){
    ...
    if (communicateStage == 2){
      ...
      for (i = first; i < last; i++) {
          force->wfunc[i] = buf[m++];
          force->grad_wfunc[i][0] = buf[m++];
          force->grad_wfunc[i][1] = buf[m++];
          force->grad_wfunc[i][2] = buf[m++];
          force->numdpneigh[i] = (int)buf[m++];
          char nptrname[30];
          sprintf(nptrname, "force:dpneigh_%d", i);
          if (Updatenmax)
              memory->create(force->dpneigh[i], force->numdpneigh[i], nptrname);
          else
              memory->grow(force->dpneigh[i], force->numdpneigh[i], nptrname);
          sprintf(nptrname, "force:grad_dpneigh_%d", i);
          if (Updatenmax)
              memory->create(force->grad_dpneigh[i], force->numdpneigh[i]*3, nptrname);
          else
              memory->grow(force->grad_dpneigh[i], force->numdpneigh[i]*3, nptrname);
          for (int kk = 0; kk < force->numdpneigh[i]; kk++){
             force->dpneigh[i][kk] = (int)buf[m++];
             force->grad_dpneigh[i][kk*3+0] = buf[m++];
             force->grad_dpneigh[i][kk*3+1] = buf[m++];
             force->grad_dpneigh[i][kk*3+2] = buf[m++];
          }
      }
    }
  }
...
int PairMixture::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;

  if (communicateStage == 3){
    m = 0;
    last = first + n;
    for (i = first; i < last; i++) {
        buf[m++] = force->f_deepmd[3*i+0];
        buf[m++] = force->f_deepmd[3*i+1];
        buf[m++] = force->f_deepmd[3*i+2];
    }
    return m;
  }
}
...
void PairMixture::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  ...
  if (communicateStage == 3){
    ...
    for (i = 0; i < n; i++) {
      j = list[i];
      force->f_deepmd[3*j+0] += buf[m++];
      force->f_deepmd[3*j+1] += buf[m++];
      force->f_deepmd[3*j+2] += buf[m++];
    }
  }
}


void PairMixture::compute(int eflag, int vflag)
{
  int i,j,m,n;
  ...
  // if (neighbor->ago == 0)
      build_wfunc();
  std::cout<<"force->ndeepmd_region = "<<force->ndeepmd_region<<std::endl;
  timestep_pm ++;
  ...
  //   if(force->wfunc[i] > 0.0) continue;
  //   for (int jj = 0; jj < jnum; jj++) {
  //     int j = jlist[jj];
  //     // if(force->wfunc[j] == 1.0) error->all(FLERR, "buff, dp_region not covered");
  //     if(force->wfunc[j] > 0.0){
  //         force->neiofbuf[i] = 1;
  //         force->nhalfdeepmd_region ++;
  //         break;
  //     }
  //   }
  // }
  ...
  printf("eng_vdwl = %20.10f    %20.10f\n", styles[0]->eng_vdwl, styles[1]->eng_vdwl);
  ...
  for (int ii = 0; ii < atom->nlocal + atom->nghost; ++ii){
    for (int dd = 0; dd < 3; ++dd){
          atom->f[ii][dd] = atom->f[ii][dd] + force->f_deepmd[ii*3+dd] + force->f_rep[ii][dd];
    }
  }
  ...
  // ago++;
  // if (ago >= neighbor->delay && ago % neighbor->every == 0) {
  //   if (neighbor->build_once) nflag = 0;
  //   if (neighbor->dist_check == 0) nflag = 1;
  //   nflag = neighbor->check_distance();
  // } else nflag = 0;
...
void PairMixture::add_tally_callback(Compute *ptr)
{
  for (int m = 0; m < nstyles; m++)
...
void PairMixture::del_tally_callback(Compute *ptr)
{
  for (int m = 0; m < nstyles; m++)
...
void PairMixture::compute_inner()
{
  for (int m = 0; m < nstyles; m++)
...
void PairMixture::compute_middle()
{
  for (int m = 0; m < nstyles; m++)
...
void PairMixture::compute_outer(int eflag, int vflag)
{
  outerflag = 1;
  compute(eflag,vflag);
  outerflag = 0;
...
/* ------------------------------------------------------------------------- */
/*
void PairMixture::allocate_pages()
{
  int create = 0;
  ...
  if (create) {
    ...
    dpage_w = new MyPage<double>[nmypage];
    for (int i = 0; i < nmypage; i++) {
      dpage_w[i].init(dnum*oneatom,dnum*pgsize);
    }
  }
}
*/
...
void PairMixture::settings(int narg, char **arg)
{
  ...
  memory->create(id_coordatom, n, "pair_mixture::id_coordatom");
  strcpy(id_coordatom,arg[0]);
  int icoordatom = modify->find_compute(id_coordatom);
  if (icoordatom < 0){ 
      error->all(FLERR,"pair_mixture must be defined after compute coord/atom");
  } else {
      coord_thres = force->numeric(FLERR,arg[1]);
      thick_trans = force->numeric(FLERR,arg[2]);
      c_coordatom = (ComputeCoordAtom*)(modify->compute[icoordatom]);
  }
  ...
  nstyles = 0;
  while (iarg < narg) {
    if (nstyles > 2) error->all(FLERR, "pair_mixture can only accept two substyles");
    if (nstyles == 0)
        if (strcmp(arg[iarg], "deepmd") != 0)
            error->all(FLERR, "The fist substyle must be deepmd");

    if (strncmp(arg[iarg],"mixture",7) == 0)
      error->all(FLERR,"Pair style mixture cannot have mixture as an argument");
    if (strcmp(arg[iarg],"none") == 0)
      error->all(FLERR,"Pair style mixture cannot have none as an argument");
...
void PairMixture::coeff(int narg, char **arg)
{
  if (narg < 3) error->all(FLERR,"Incorrect args for pair coefficients");
  if (!allocated) allocate();

  if (lmp->kokkos)
    error->all(FLERR,"Cannot yet use pair mixture with Kokkos");
  ...
  if (m == nstyles) {
    ...
    else error->all(FLERR,"Pair coeff for mixture has invalid style");
  }
...
void PairMixture::init_style()
{
  ...
  for (istyle = 0; istyle < nstyles; istyle++) {
    ...
    if (used == 0) error->all(FLERR,"Pair mixture sub-style is not used");
  }
    ...
    if (special_lj[istyle]) {
      for (i = 1; i < 4; ++i) {
        if (((force->special_lj[i] == 0.0) || (force->special_lj[i] == 1.0))
          ...
          error->all(FLERR,"Pair_modify special setting for pair mixture "
  ...
  for (i = 0; i < neighbor->nrequest; i++) {
    ...
    int **ijskip;
    memory->create(ijskip,ntypes+1,ntypes+1,"pair_mixture:ijskip");
  ...
  ssa = neighbor->requests[irequest]->ssa;
...
double PairMixture::init_one(int i, int j)
{
  ...
  for (int k = 0; k < nmap[i][j]; k++) {
    ...
    std::cout<<"cutcutcut == "<<cut<<std::endl;
  ...
  cutmax = MAX(cutmax, thick_trans);

  std::cout<<"cutcutcutmax == "<<cutmax<<std::endl;
  return cutmax;
}
...
void PairMixture::write_restart(FILE *fp)
{
...
void PairMixture::read_restart(FILE *fp)
{
  int me = comm->me;
...
double PairMixture::single(int i, int j, int itype, int jtype,
  ...
  for (int m = 0; m < nmap[itype][jtype]; m++) {
    if (rsq < styles[map[itype][jtype][m]]->cutsq[itype][jtype]) {
      if (styles[map[itype][jtype][m]]->single_enable == 0)
        error->one(FLERR,"Pair mixture sub-style does not support single call");

      if ((special_lj[map[itype][jtype][m]] != NULL) ||
          (special_coul[map[itype][jtype][m]] != NULL))
        error->one(FLERR,"Pair mixture single calls do not support"
...
void PairMixture::modify_params(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"pair") == 0) {
    ...
    if (m == nstyles) error->all(FLERR,"Unknown pair_modify mixture sub-style");
    int iarg = 2;
    ...
    if (multiple[m]) {
      ...
      if (m == nstyles)
        error->all(FLERR,"Unknown pair_modify mixture sub-style");
      iarg = 3;
    }
...
void PairMixture::modify_special(int m, int /*narg*/, char **arg)
{
  double special[4];
  int i;
...
void PairMixture::set_special(int m)
{
  int i;
  if (special_lj[m])
...
double * PairMixture::save_special()
{
...
void PairMixture::restore_special(double *saved)
{
  for (int i = 0; i < 4; ++i) {
...
void *PairMixture::extract(const char *str, int &dim)
{
  void *cutptr = NULL;
  void *ptr;
  double cutvalue = 0.0;
  ...
  for (int m = 0; m < nstyles; m++) {
    ...
    if (ptr && strcmp(str,"cut_coul") == 0) {
      if (cutptr && dim != couldim)
        error->all(FLERR,
                   "Coulomb styles of pair mixture sub-styles do not match");
      double *p_newvalue = (double *) ptr;
      double newvalue = *p_newvalue;
      if (cutptr && (newvalue != cutvalue))
        error->all(FLERR,
                   "Coulomb cutoffs of pair mixture sub-styles do not match");
      if (dim == 0) {
        cutptr = ptr;
        cutvalue = newvalue;
      }
...
int PairMixture::check_ijtype(int itype, int jtype, char *substyle)
{
  for (int m = 0; m < nmap[itype][jtype]; m++)
...
double PairMixture::memory_usage()
{
