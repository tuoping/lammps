...
PairNNP::~PairNNP()
{
  ...
  if (ndeepmd_region > 0) memory->destroy(ilist_short);
  if (n_label_reverse > 0) memory->destroy(ilist_label_reverse);
  if (maxatom > 0){
    memory->sfree(firstneigh_new);
    memory->destroy(numneigh_new);
  }
}
...
void PairNNP::compute(int eflag, int vflag)
{
  ...
  double **x = atom->x;
  double **f = atom->f;
  double *f_deepmd = force->f_deepmd;
  int *type = atom->type;
  int nlocal = atom->nlocal;
  int nghost = 0;
  if (do_ghost) {
  ...
  if (ago_deepmd != force->ndeepmd_region) ago = 0;
  if (ago_halfdeepmd != force->nhalfdeepmd_region) ago = 0;
  cout<<"ago = "<<ago<<endl;
  ago_deepmd = force->ndeepmd_region;
  ago_halfdeepmd = force->nhalfdeepmd_region;

  std::cout<<"ndeepmd_region = "<<force->ndeepmd_region<<std::endl;
  std::cout<<"nhalfdeepmd_region = "<<force->nhalfdeepmd_region<<std::endl;
  vector<double > dcoord ((nlocal + nghost) * 3, 0.);
  vector<int > dtype (nlocal + nghost);
  vector<int > fringe_list (nlocal - force->ndeepmd_region);
  ...
  std::cout<<"nmax = "<<atom->nmax<<std::endl;
  std::cout<<"inum = "<<inum<<std::endl;
  if (inum != nlocal) {
      error->all(FLERR, "inum != nlocal");
  }
  
  int nlocal_long = nlocal;
  int inum_short = 0;
  int nlocal_short = 0;
  int n_fringe = 0;
  
  MyPage<int> *ipage = list->ipage;
  // if (force->ndeepmd_region + force->nhalfdeepmd_region > 0){
  std::cout<<"yes_amm = "<<force->yes_amm<<std::endl;
  if (force->yes_amm == 1){
     // compute
     if (ndeepmd_region < force->ndeepmd_region + force->nhalfdeepmd_region){
         if (ndeepmd_region > 0) memory->destroy(ilist_short);
         ndeepmd_region = force->ndeepmd_region + force->nhalfdeepmd_region;
         memory->create(ilist_short,ndeepmd_region,"pair_nnp:ilist_short");
     }
     if (n_label_reverse < nlocal){
         if (n_label_reverse > 0) memory->destroy(ilist_label_reverse);
         n_label_reverse = nlocal;
         memory->create(ilist_label_reverse,nlocal,"pair_nnp:ilist_label_reverse");
     }
     if (maxatom < atom->nmax){
         if(maxatom > 0){
           memory->sfree(firstneigh_new);
           memory->destroy(numneigh_new);
         }
         maxatom = atom->nmax;
         firstneigh_new = (int **) memory->smalloc(maxatom*sizeof(int *),
                                        "pair_nnp:firstneigh_new");
         memory->create(numneigh_new,maxatom,"pair_nnp:numneigh_new");
     }
     for (int ii=0; ii<inum; ++ii){
         int i = ilist[ii];
         if (wfunc[i] > 0.0 ){
           // add to new lists including dcoord, dtype, ilist, numneigh, firstneigh
           for (int dd = 0; dd < 3; ++dd){
               dcoord[nlocal_short*3 + dd] = x[i][dd] - domain->boxlo[dd];
           }
           dtype[nlocal_short] = type[i] - 1;
     
           ilist_label_reverse[i] = inum_short;
           ilist_short[inum_short] = i;
           firstneigh_new[i] = list->firstneigh[i];
           numneigh_new[i] = list->numneigh[i];
     
           nlocal_short ++; 
           inum_short ++;
           
         } else{
             if (force->neiofbuf[i] == 1) {
               for (int dd = 0; dd < 3; ++dd){
                   dcoord[nlocal_short*3 + dd] = x[i][dd] - domain->boxlo[dd];
               }
               dtype[nlocal_short] = type[i] - 1;
               ...
               numneigh_new[i] = n;
               firstneigh_new[i] = neighptr;
               ipage->vgot(n);
               if (ipage->status())
                 error->one(FLERR,"NNP: Neighbor list overflow, boost neigh_modify one");

               nlocal_short ++; 
               inum_short ++;

               for (int jj = 0; jj < numneigh_new[i]; ++jj){
                   int j = firstneigh_new[i][jj];
               }
             }else {
                  if(n_fringe > nlocal - force->ndeepmd_region - force->nhalfdeepmd_region) error->all(FLERR, "wrong n_fringe");
                  fringe_list[n_fringe] = i;
                  n_fringe ++;
             }
         }
     }
     if(nlocal_short != (force->ndeepmd_region + force->nhalfdeepmd_region)){
         cout<<"nlocal_short "<<nlocal_short<<" != "<<"ndeepmd_region "<<force->ndeepmd_region<<endl;
         error->all(FLERR, "Wrong nlocal_short");
     }
     
     
     int nall_short = nlocal_short;
     for (int ii = 0; ii < n_fringe; ++ii){
       int i = fringe_list[ii];
       ilist_label_reverse[i] = nlocal_short + ii;
       for (int dd = 0; dd < 3; ++dd){
           dcoord[nall_short*3 + dd] = x[i][dd] - domain->boxlo[dd];
       }
       dtype[nall_short] = type[i] - 1;
       nall_short ++;
     }
     cout<<"nloc + n_fringe = "<<nall_short<<endl;
     if (nall_short != nlocal) error->all(FLERR, "nlocal_short + n_fringe != nlocal");
     for (int ii = 0; ii < nghost; ++ii){
       for (int dd = 0; dd < 3; ++dd){
           dcoord[nall_short*3 + dd] = x[ii+nlocal][dd] - domain->boxlo[dd];
       }
       dtype[nall_short] = type[ii+nlocal] - 1;
       nall_short ++;
     }
     
     nlocal = nlocal_short;
     nall = nall_short;
     nghost = nall_short - nlocal_short;
     inum = inum_short;
     std::cout<<"inum_short = "<<inum_short<<std::endl;
     std::cout<<"nlocal_short = "<<nlocal_short<<std::endl;
     std::cout<<"nall_short = "<<nall_short<<std::endl;

  } else{
      // get coord
      numneigh_new = list->numneigh;
      firstneigh_new = list->firstneigh;
...
// uniform aparam
  ...
  if (do_ghost) {
        int *ilist_lmp;
        // if (force->ndeepmd_region > 0)
        if (force->yes_amm == 1)
            // LammpsNeighborList lmp_list (inum_short, nlocal_long, ilist_short, numneigh, firstneigh, ilist_label_reverse);
            ilist_lmp = ilist_short;
        else
            // LammpsNeighborList lmp_list (list->inum, nlocal_long, list->ilist, list->numneigh, list->firstneigh, ilist_label_reverse);
            ilist_lmp = list->ilist;
        LammpsNeighborList lmp_list (inum, nlocal_long, ilist_lmp, numneigh_new, firstneigh_new, ilist_label_reverse);
  ...
  if (force->yes_amm == 1){
  // for (int ii = 0; ii < nall; ++ii){
      for (int i = 0; i < nlocal_long; ++i){
        if(force->neiofbuf[i] == 1) continue;
        int ii = ilist_label_reverse[i];
        for (int dd = 0; dd < 3; ++dd){
          // f[ii][dd] += scale[1][1] * dforce[3*ii+dd];
          f_deepmd[i*3 + dd] += scale[1][1] * dforce[3*ii+dd];
        }
      }
      for (int i = nlocal_long; i < nall; ++i){
        for (int dd = 0; dd < 3; ++dd){
          f_deepmd[i*3 + dd] += scale[1][1] * dforce[3*i+dd];
        }
      }
...
// int PairNNP::pack_forward_comm(int n, int *list, double *buf,
...
// 
// 
// void PairNNP::unpack_forward_comm(int n, int first, double *buf)
// {
//   int i,m,last;
// 
//   m = 0;
